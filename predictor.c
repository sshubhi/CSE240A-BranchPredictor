...
{	uint8_t mask = 2;
	/*if (cntr == gshare_bht) {
		printf("\ncntr is %d and index is %d", cntr,index);
		printf("\n2bht is %d", *(cntr+index)); 
		printf("\nprediction is %d",(*(cntr+index) & mask)>>1);
	} */
	printf("\nget_prediction. cntr is %d", cntr);	
...
init_predictor()
{
  ...
  ghr = 0;
  gshare_bht = (uint8_t *)calloc(1, (globalhist_mask + 1)*sizeof(uint8_t));
  ...
  printf("\ninit_prediction");
  printf("\n gshare_bht is: %d, localpred_bht is: %d, globalpred_bht is: %d, choice_predictor is:%d",gshare_bht,localpred_bht,globalpred_bht,choice_predictor);
}
...
{	uint8_t	prediction;
    ...
    //Gshare
	uint32_t pc_xor_ghr = (pc ^ ghr) & globalhist_mask;
	//printf ("\n pc is %d, ghr is %d, xor is %d, mask is %d", pc,ghr,pc_xor_ghr,globalhist_mask);
	//printf("\n pc_index_mask is %d, pc is %d, pc_xor_ghr is %d", pc_index_mask, pc, pc_xor_ghr);
	//printf ("\n Other masks: %d, %d", globalhist_mask, localhist_mask);
	printf("\nmake_prediction");
	//Tournament
...
{	
	...
	uint32_t localpred_index = get_local_pred_pattern(localhist_table,pc_index);
    uint32_t globalpred_index = ghr & globalhist_mask;
	printf("\n update_predictor index is %d",pc_xor_ghr);
